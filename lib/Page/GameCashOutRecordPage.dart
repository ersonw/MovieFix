...
class _GameCashOutRecordPage extends State<GameCashOutRecordPage> {
  ...
  _getList() async {
    ...
    if (result['list'] != null) {
      List<GameCashOut> list =
          (result['list'] as List).map((e) => GameCashOut.formJson(e)).toList();
      if (page > 1) {
  ...
  Widget build(BuildContext context) {
    return GeneralRefresh(
      ...
      title: '充值记录',
      body: _records.isNotEmpty && refresh == false
          ? null
          : Container(
                  ...
                  InkWell(
                    onTap: () async {
                      await Clipboard.setData(
                          ClipboardData(text: record.orderNo));
              ...
              child: Row(
                ...
                children: [
                  Flexible(
                      child: Container(
                    child: Text(
                      Global.getTimeToString(record.addTime),
                      style: TextStyle(color: Colors.white.withOpacity(0.6)),
                    ),
                  )),
                  Flexible(
                      child: Container(
                    child: Text(
                      Global.getTimeToString(record.updateTime),
                      style: TextStyle(color: Colors.white.withOpacity(0.6)),
                    ),
                  )),
                ],
